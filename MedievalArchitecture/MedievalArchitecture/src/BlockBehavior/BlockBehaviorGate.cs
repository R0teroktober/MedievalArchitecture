...
public class BlockBehaviorGate : StrongBlockBehavior, IMultiBlockColSelBoxes, IMultiBlockBlockProperties
{
    ...
    private BEBehaviorGate GetBE(IBlockAccessor blockAccessor, BlockPos pos)
    {
        return blockAccessor.GetBlockEntity(pos)?.GetBehavior<BEBehaviorGate>();
    }

    private BEBehaviorGate GetRootBE(IBlockAccessor blockAccessor, BlockPos childPos, Vec3i offsetInv)
    {
        ...
        ref EnumHandling handling)
    {
        ...
        GetBE(ba, rootPos)?.SetPlacementData(rotDeg);
    ...
    private int GetPlacementRotDeg(BlockSelection blockSel)
    {
        var center = blockSel.Face.ToAB(blockSel.Face.PlaneCenter);
        var hit = blockSel.Face.ToAB(blockSel.HitPosition.ToVec3f());

        int rotDeg = (int)Math.Round(
            GameMath.RAD2DEG * Math.Atan2(center.A - hit.A, center.B - hit.B) / 90f
        ) * 90;

        return GameMath.Mod(rotDeg, 360);
    }
    ...
    public void IterateOverEach(BlockPos rootPos, int rotDeg, ActionConsumable<BlockPos> onBlock)
    {
        ...
        int rot = GameMath.Mod(rotDeg, 360);
    ...
    {
        switch (GameMath.Mod(rotDeg, 360))
        {
    ...
    private float GetLiquidBarrierHeight(BEBehaviorGate be, BlockFacing face)
    {
    ...
    private int GetRetention(BEBehaviorGate be, BlockPos pos, BlockFacing facing, EnumRetentionType type)
    {
        if (be == null) return 0;
